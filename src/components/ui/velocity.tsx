...
function ParallaxText({ children, baseVelocity = 100 }: ParallaxProps) {
   ...
   * calculate
   */
  const x = useTransform(baseX, (v) => `${wrap(-20, -48, v)}%`);
  ...
  return (
    <div className="parallax">
      <motion.div className="scroller" style={{ x, fontSize: "50px", fontWeight: "bold",fontFamily:"Poppins", color: "black", whiteSpace: "nowrap" }}>
        <span>{children} </span>
        <span>{children} </span>
        <span>{children} </span>
        <span>{children} </span>
